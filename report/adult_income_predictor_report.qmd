...
# EDA Analysis
## Data Summary
In @tbl-despriction, it describes the number of unique categories and the missing values for the relevant categorical variables.

```{python}
#| label: tbl-despriction
#| tbl-cap: description table for categorical variables
import pandas as pd
from tabulate import tabulate

data_adult = pd.read_csv("../data/processed/cleaned_data.csv")
categorical_columns = ['marital-status', 'relationship', 'occupation', 'workclass', 'race', 'sex']
feature_summary = []
for col in categorical_columns:
    unique_values = data_adult[col].nunique() 
    missing_values = data_adult[col].isnull().sum() 
    missing_percentage = (missing_values / len(data_adult)) * 100  
...
# Markdown(feature_summary_df.to_markdown(index = False))
print(tabulate(feature_summary_df, headers='keys', tablefmt='pipe', showindex=False))
```
...
test_score = test_score_df.loc[0, 'test_score']
```
